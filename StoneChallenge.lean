...
/-!
# Spectral and Stone theorems (Challenge)

This module states the projection-valued-measure form of the spectral theorem.
For a self-adjoint partial linear operator `A`, it gives a real PVM whose
scalar spectral measures determine exactly the domain of `A` by their second
moments and determine `A` by their first moments. The PVM is unique on Borel
sets.

It also states Stone's correspondence between self-adjoint operators and
strongly continuous one-parameter unitary groups. Only the two advertised
theorem proofs are omitted; all boundary definitions are complete.
-/
